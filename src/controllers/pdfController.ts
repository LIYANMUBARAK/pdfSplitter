...
export async function sendEmailWebhook(req:Request,res:Response){
    ...
    var policy = req.body.customData.policy
    ...
    if (!toEmail || !subject || !pdfUrl || !locationId || !claim  || !homeOwner || !first_name || !last_name) {
      ...
      if(!claim){
        ...
        if (!policy || isNaN(Number(policy)) || policy.trim() === "") {
          console.log("entered not policy oresent condition")
